...
set "HERE=%~dp0"
set "RUNNER=%PROJECT%\Tools\RunStage72D.ps1"

echo INDUSTRY KICK - STAGE 7.2D
echo POWERSHELL QUOTE FIX ONLY
echo NO DSP / VALIDATOR / PROJECT SOURCE CHANGES
echo.

if not exist "%PROJECT%\Faust\IndustryKickV2_R4_Freeze.dsp" goto :missing
if not exist "%PROJECT%\Tests\DspSmoke.cpp" goto :missing

copy /Y "%HERE%Payload\Tools\RunStage72D.ps1" "%RUNNER%" >nul
...
echo Exit code: %RC%
echo Bundle:
echo "%PROJECT%\Stage72DValidationBundle.zip"
echo.
...
echo Upload Stage72DValidationBundle.zip.
pause
exit /b %RC%
...
:copyfail
echo ERROR: could not install Stage 7.2D runner.
pause
exit /b 1
...
echo Stage 7.2D was NOT executed.
echo No build/DSP result can be inferred.
pause
exit /b 90
