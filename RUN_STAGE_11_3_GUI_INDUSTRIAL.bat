...
set "PACKAGE=%~dp0"
set "VALID=%PROJECT%\Stage113Validation"
set "BUNDLE=%PROJECT%\Stage113ValidationBundle.zip"
set "BACKUP=%PROJECT%\Checkpoints\Before_Stage11_3"

set "PRESET_FIX=NOT_TESTED"
...
echo INDUSTRY KICK - STAGE 11.3 GUI INDUSTRIAL
echo Industrial GUI restyle. DSP and parameters frozen.
echo.
...
echo [0/8] APPLY PRESET BALANCE FIX (idempotent)
powershell -NoProfile -ExecutionPolicy Bypass -File "%PACKAGE%Tools\ApplyGuiIndustrialPatchD.ps1" > "%VALID%\PresetFix.log" 2>&1
if errorlevel 1 (
  set "PRESET_FIX=FAIL"
  set "FAILURE_CLASS=PRESET"
  set "FAILED_STEP=ApplyGuiIndustrialPatch.ps1"
  goto :FAIL
)
set "PRESET_FIX=PASS"
call :WriteStatus
...
if not exist "%PACKAGE%Reference\FactoryPresets_STAGE112.h" (
  set "FAILURE_CLASS=SOURCE"
  set "FAILED_STEP=Reference\FactoryPresets_STAGE112.h missing"
  goto :FAIL
)
fc /b "%PROJECT%\Source\FactoryPresets.h" "%PACKAGE%Reference\FactoryPresets_STAGE112.h" >nul
if errorlevel 1 goto :SOURCE_FAIL
fc /b "%PROJECT%\Source\PluginEditor.cpp" "%PACKAGE%Reference\PluginEditor_STAGE113.cpp" >nul
if errorlevel 1 goto :SOURCE_FAIL
...
if errorlevel 1 (
  if exist "C:\Program Files\plugival\pluginval.exe" (
    "C:\Program Files\plugival\pluginval.exe" --validate "%VST3PATH%" --output-dir "%VALID%" > "%VALID%\pluginval.log" 2>&1
    if errorlevel 1 (
      set "PLUGINVAL=FAIL"
  ...
  pluginval --validate "%VST3PATH%" --output-dir "%VALID%" > "%VALID%\pluginval.log" 2>&1
  if errorlevel 1 (
    set "PLUGINVAL=FAIL"
    set "FAILURE_CLASS=HOST"
    set "FAILED_STEP=pluginval validation"
...
echo.
echo ================================================================
echo STAGE_11_3_GUI_INDUSTRIAL=PASS
echo PRESET_FIX=%PRESET_FIX%
...
set "FAILURE_CLASS=SOURCE"
set "FAILED_STEP=Project source outside approved Stage 11.2 checkpoint"
goto :FAIL
...
echo.
echo ================================================================
echo STAGE_11_3_GUI_INDUSTRIAL=FAIL
...
:WriteStatus
> "%VALID%\STATUS.txt" echo PRESET_FIX=%PRESET_FIX%
