...
int main()
{
    ...
    std::cout << "presetAuditStage=10.3\n";
    ...
    const bool bankShape = names.size() == 250 && uniqueNames.size() == 250;
    ok = ok && bankShape;
    std::cout << "factoryBank=" << bankShape << " count=" << names.size() << "\n";

    {
        KickcrafterAudioProcessor bankMap;
        constexpr std::array<int,5> indices { 0, 10, 20, 30, 40 };
        bool bankMapOk=true;
        for(int bank=0;bank<5;++bank)
        {
            bankMap.loadFactoryPreset(indices[(size_t)bank]);
            bankMapOk=bankMapOk && bankMap.getAccentBank()==bank;
        }
        ok=ok && bankMapOk;
        std::cout << "accentBankMapping=" << bankMapOk << "\n";
    }

    // ---------------------------------------------------------------------
    // Approved A anchors remain the first CORE preset of each 50-preset family.
    // Technical checks are performed at several rates and buffer sizes.
    // ---------------------------------------------------------------------
    constexpr std::array<int, 5> anchorIndices { 0, 50, 100, 150, 200 };
    ...
    // ---------------------------------------------------------------------
    // Stage 10.3: full 250-preset technical pass + objective audit export.
    // Existing technical/residual thresholds are unchanged.
    // The CSV is evidence for later Python analysis; it is not a perceptual score.
    // ---------------------------------------------------------------------
    {
        const auto auditDir = juce::File::getCurrentWorkingDirectory().getChildFile("Stage103PresetAudit");
        if (auditDir.exists())
            auditDir.deleteRecursively();
        const bool auditDirOk = auditDir.createDirectory().wasOk();

        std::ofstream auditCsv(
            auditDir.getChildFile("preset_metrics.csv").getFullPathName().toStdString());

        bool auditExportOk = auditDirOk && (bool)auditCsv;
        if (auditCsv)
            auditCsv
                << "index,name,family,bank,technical,residual,peak,rms,crest075_db,"
                << "safety_hit_pct,dc,w0_30_db,w30_80_db,w80_180_db,w180_400_db,"
                << "w400_750_db,low80_180_db,low180_400_rel_db,low400_750_rel_db,"
                << "near_peak90_075_pct,tail_rms_db,tail_mean,decay_drop_db\n";

        const char* familyLabels[] = { "ROUND", "PUNCH", "HARD", "INDUSTRIAL", "RAVE" };
        const char* bankLabels[] = { "CORE", "TIGHT", "HEAVY", "SUSTAIN", "MUTANT" };
        ...
        for (int i = 0; i < names.size(); ++i)
        {
            ...
            const int family = juce::jlimit(0, 4, i / 50);
            const int bank = juce::jlimit(0, 4, (i % 50) / 10);
            const double lowReference = lowBandRmsDbWindow(r, 48000.0, 0.080, 0.180);

            if (auditCsv)
            {
                auto csvName = names[i].replace(",", ";");
                auditCsv
                    << i << ","
                    << csvName << ","
                    << familyLabels[family] << ","
                    ...
                    << r.metrics.peak << ","
                    << r.metrics.rms << ","
                    << crestDbWindow(r, 48000.0, 0.0, 0.750) << ","
                    << r.metrics.safetyCeilingHitPercent << ","
                    << r.metrics.dc << ","
                    ...
                    << rmsDbWindow(r, 48000.0, 0.400, 0.750) << ","
                    << lowReference << ","
                    << lowBandRmsDbWindow(r, 48000.0, 0.180, 0.400) - lowReference << ","
                    << lowBandRmsDbWindow(r, 48000.0, 0.400, 0.750) - lowReference << ","
                    << nearPeakWindowPercent(r, 48000.0, 0.0, 0.750, 0.90) << ","
                    << rmsDbWindow(r, 48000.0, 6.5, 7.0) << ","
                    << meanWindow(r, 48000.0, 6.5, 7.0) << ","
                    << residualDecayDropDb(r, 48000.0)
                    << "\n";
            }

            if (i % 10 == 0)
            {
                auto safeName = names[i].replace("/", "-").replace("\\\\", "-").replace(":", "-");
                const auto wav = auditDir.getChildFile(
                    juce::String(i).paddedLeft('0', 3) + "_"
                    + familyLabels[family] + "_" + bankLabels[bank] + "_"
                    + safeName + ".wav");
                auditExportOk = writeMonoWav(wav, r, 48000.0) && auditExportOk;
            }
        ...
        auditExportOk = auditExportOk && auditCsv.good();
        ok = ok && allPresets && auditExportOk;
        std::cout << "allFactoryPresetsTechnical=" << allPresets << "\\n";
        std::cout << "factoryPresetAuditExport=" << auditExportOk << "\\n";
    }
    ...
    // ---------------------------------------------------------------------
    {
        ...
        for (int i = 0; i < 25; ++i)
        {
            ...
            const bool basicTechnical = technicalPass(r);
            const bool residualSafe = residualSafetyPass(r, 48000.0);
            const bool bankValid = p.getAccentBank() >= 0 && p.getAccentBank() <= 4;
            const bool pass = basicTechnical && residualSafe && bankValid;
    ...
    // ---------------------------------------------------------------------
    {
        ...
        const bool sameAccentBank = source.getAccentBank() == restored.getAccentBank();
        same = same && sameAccentBank;
        ok = ok && same;
        std::cout << "accentBankState=" << sameAccentBank << "\n";
