...
int main()
{
    ...
    // User-approved Stage 6.6/6.6B envelope SHAPES relative to 80-180 ms.
    // Gain-independent: output calibration cannot game this test.
    constexpr std::array<std::array<double,5>,5> approvedEnvelopeShapes {{
        {{  0.9822,  0.3188, 0.0, -0.7275,  -4.0926 }}, // ROUND
        {{  4.9202,  3.7375, 0.0, -7.8909, -36.5148 }}, // PUNCH
        {{  1.6443,  1.0550, 0.0, -1.1897,  -4.5715 }}, // HARD less-sat
        {{  0.8838,  0.4579, 0.0, -1.1580,  -3.0967 }}, // INDUSTRIAL less-sat
        {{ -1.1653, -0.2191, 0.0, -2.0733, -12.1998 }}  // RAVE
    }};
    ...
    // ---------------------------------------------------------------------
    {
        ...
        // Stage69B showed ROUND/PUNCH at 0.827 despite a >29 dB difference
        // in the 650-750 ms decay window. Approved per-family envelope-shape
        // parity above is the actionable sonic gate.
        ok = ok && routed;
    ...
    // ---------------------------------------------------------------------
    {
        const auto renderDir = juce::File::getCurrentWorkingDirectory()
                                   .getChildFile("Stage610TestRenders");
