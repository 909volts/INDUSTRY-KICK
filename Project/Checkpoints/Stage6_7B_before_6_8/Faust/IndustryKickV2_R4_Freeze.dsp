...
// INDUSTRY KICK V2 — Stage 6.7 compiled family-diversity candidate
...
roundAmp = ampEnv(roundDecay)
           * (1.0 + mix(0.34,0.74,punch)*transEnv(mix(0.045,0.024,punch)));
...
punchDecay = min(0.20,max(0.10,(length/1000.0)*0.72));
punchAmp = ampEnv(punchDecay)
           * (1.0 + mix(1.35,2.55,punch)*transEnv(mix(0.028,0.016,punch)));
...
raveAmp = ampEnv(raveDecay)
          * (1.0 + mix(0.74,1.55,punch)*transEnv(mix(0.024,0.012,punch)));
