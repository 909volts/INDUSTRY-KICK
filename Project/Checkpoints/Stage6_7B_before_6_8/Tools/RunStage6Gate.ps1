...
Write-Host "INDUSTRY KICK V2 - Stage 6.7B Residual Validation Build Gate"
...
Write-Host ""
Write-Host "STAGE_6_7B_BUILD_GATE_COMPLETE"
$renderDir = Join-Path $ProjectRoot "Stage67TestRenders"
$renderZip = Join-Path $ProjectRoot "Stage67TestRenders.zip"
if (Test-Path $renderDir) {
    ...
    Write-Host "STAGE_6_7_RENDER_ZIP=$renderZip"
} else {
    Write-Host "STAGE_6_7_RENDER_ZIP=NOT_FOUND"
}
Write-Host "Send Stage67TestRenders.zip only if result=PASS."
