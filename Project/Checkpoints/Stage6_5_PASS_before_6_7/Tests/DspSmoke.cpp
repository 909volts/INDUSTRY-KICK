...
{
    const double tailRmsDb = rmsDbWindow(r, sampleRate, 3.5, 4.0);
    const double tailMean  = meanWindow(r, sampleRate, 3.5, 4.0);

    return std::isfinite(tailRmsDb)
        && std::isfinite(tailMean)
        && tailRmsDb < -50.0
        && std::abs(tailMean) < 1.0e-4;
}
                               ...
                               const std::array<double, 5>& targetDb)
{
    ...
    const std::array<double,5> tolerances {{ 2.0, 2.5, 3.0, 4.0, 4.0 }};
...
int main()
{
    ...
    constexpr std::array<std::array<double,5>,5> approvedEnvelopeTargets {{
        {{ -3.5240, -3.5976, -4.9297, -7.4567,  -14.7670 }},
        {{ -3.1377, -3.5138, -7.1055, -13.7886, -34.9029 }},
        {{ -1.5952, -2.2157, -3.4042, -5.4618,  -12.0037 }},
        {{ -3.2073, -3.4523, -4.1347, -6.2827,  -13.3846 }},
        {{ -3.0987, -3.6127, -5.1416, -9.1872,  -24.6901 }}
    }};
    ...
    for (const auto presetIndex : anchorIndices)
    {
        ...
        const auto r = render(p, 48000.0, 256, 4.0, 0);
        ...
        std::cout << "anchorResidual preset=" << presetIndex
                  << " pass=" << pass
                  << " tailRmsDb=" << rmsDbWindow(r,48000.0,3.5,4.0)
                  << " tailMean=" << meanWindow(r,48000.0,3.5,4.0)
                  << "\n";
    }
