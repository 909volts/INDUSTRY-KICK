    ...
    float a0, float a1, juce::Slider& s)
{
    ...
    juce::ColourGradient bezelGrad(juce::Colour(0xff8a8e91),
                                    c.x - radius, c.y - radius,
                                    juce::Colour(0xff161a1e),
...
void KickcrafterAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    juce::ColourGradient chassis(juce::Colour(0xff24282b), 0.0f, 0.0f,
                                 juce::Colour(0xff070809), baseW, baseH, true);
                     ...
                     const juce::String& subtitle, bool masterPanel = false)
    {
        ...
        g.setColour(juce::Colour(0xff303538));
        g.fillRoundedRectangle(r, 3.0f);
        g.setColour(black);
        g.fillRoundedRectangle(r.reduced(3.0f), 2.0f);

        juce::ColourGradient inner(masterPanel ? juce::Colour(0xff1b1717) : juce::Colour(0xff171b1e),
                                   x + 5.0f, y + 5.0f,
                                   juce::Colour(0xff090b0d), x + w, y + h, false);
    ...
    panel(20, 650, 1050, 250, "05", "MUTANT PRESS", "SHAPE / EVOLVE / DESTROY");
    panel(1085, 650, 335, 250, "06", "MASTER PRESS", "FIXED GLUE CHAIN", true);
    ...
    for (float cx : { 236.0f, 556.0f, 876.0f })
    {
        g.setColour(juce::Colour(0xff292e31));
        g.fillRoundedRectangle(cx - 104.0f, 676.0f, 208.0f, 180.0f, 3.0f);
        g.setColour(juce::Colour(0xff4d5356));
        g.drawRoundedRectangle(cx - 104.0f, 676.0f, 208.0f, 180.0f, 3.0f, 1.0f);
        g.setColour(juce::Colour(0xff111416));
        g.fillRect(cx - 92.0f, 689.0f, 184.0f, 5.0f);
    }

    for (int x = 1102; x < 1398; x += 17)
    {
        g.setColour(juce::Colour(0xff303538));
        g.fillRoundedRectangle((float)x, 674.0f, 9.0f, 25.0f, 2.0f);
    }
