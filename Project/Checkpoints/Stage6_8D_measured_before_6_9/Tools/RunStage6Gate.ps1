...
Write-Host "INDUSTRY KICK V2 - Stage 6.8 Audio Parity Build Gate"
...
Write-Host ""
Write-Host "STAGE_6_8_BUILD_GATE_COMPLETE"
$renderDir = Join-Path $ProjectRoot "Stage68TestRenders"
$renderZip = Join-Path $ProjectRoot "Stage68TestRenders.zip"
if (Test-Path $renderDir) {
    ...
    Write-Host "STAGE_6_8_RENDER_ZIP=$renderZip"
} else {
    Write-Host "STAGE_6_8_RENDER_ZIP=NOT_FOUND"
}
Write-Host "Send Stage68TestRenders.zip only if result=PASS."
