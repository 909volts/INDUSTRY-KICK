...
// INDUSTRY KICK V2 — R4 sonic baseline / Stage 5 Faust freeze candidate
...
roundAmp = ampEnv(min(0.48,max(0.26,length/1000.0)))
           * (1.0 + mix(0.45,0.95,punch)*transEnv(mix(0.045,0.024,punch)));
...
roundSig =
  ...
  : softNL(mix(1.25,2.35,drive),mix(0.005,0.040,shape))
...
punchAmp = ampEnv(min(0.26,max(0.14,length/1000.0)))
           * (1.0 + mix(1.25,2.35,punch)*transEnv(mix(0.034,0.020,punch)));
...
punchSource =
   ...
   +mix(0.035,0.17,metal*impact)*sineFromPhase(punchPhase,6)*transEnv(mix(0.017,0.007,punch));

punchNoise = no.noise
           : fi.highpass(2,mix(700,1300,colour))
           : fi.lowpass(2,mix(2800,5000,colour))
           : *(mix(0.012,0.075,impact*colour)*transEnv(mix(0.008,0.0035,punch)));

punchSig =
    (punchSource + punchNoise)
  : fi.peak_eq_cq(mix(2.5,7.5,impact),mix(102,138,punch),0.85)
  : softNL(mix(1.25,2.15,drive),mix(0.005,0.030,shape))
...
hardAmp = ampEnv(min(0.36,max(0.19,length/1000.0)))
          * (1.0 + mix(0.65,1.30,punch)*transEnv(mix(0.042,0.024,punch)));
...
industrialAmp = ampEnv(min(0.52,max(0.28,length/1000.0)))
                * (1.0 + mix(0.60,1.25,punch)*transEnv(mix(0.052,0.028,punch)));
...
industrialSource =
   ...
   +mix(0.04,0.22,metal)*sineFromPhase(industrialPhase,4)*expTauEnv(0.0001,mix(0.14,0.30,body))
   +mix(0.02,0.12,metal*grit)*sineFromPhase(industrialPhase,5)*expTauEnv(0.0001,mix(0.14,0.30,body))
...
industrialSig =
  ...
  : softNL(mix(1.6,3.25,drive),mix(0.04,0.18,shape))
  : fi.peak_eq_cq(mix(1.0,6.5,metal),mix(320,720,colour),0.75)
  : hardNL(mix(1.01,1.28,destroy))
  : fi.lowpass(3,mix(5000,8500,colour))
  : softNL(mix(1.02,1.55,drive),mix(0.015,0.06,shape))
...
raveAmp = ampEnv(min(0.29,max(0.15,length/1000.0)))
          * (1.0 + mix(0.8,1.7,punch)*transEnv(mix(0.030,0.016,punch)));

raveHigh = mix(0.12,0.52,metal*colour);
raveSource =
   ...
   +raveHigh*mix(0.22,0.52,destroy)*sineFromPhase(ravePhase,9))*transEnv(mix(0.012,0.0048,punch));

raveClick = no.noise
          : fi.highpass(2,mix(900,1700,colour))
          : fi.lowpass(2,mix(4800,8200,colour))
          : *(mix(0.025,0.16,impact*colour)*transEnv(mix(0.006,0.0025,punch)));
...
raveSig =
  ...
  : softNL(mix(1.35,2.55,drive),mix(0.005,0.045,shape))
  : hardNL(mix(1.00,1.20,destroy))
  : fi.high_shelf(mix(-2,1.5,colour),3200)
  : fi.lowpass(4,mix(7000,11500,colour))
  : fi.highpass(2,20)
  : softNL(mix(1.05,1.35,drive),0.0);

mono = (roundSig,punchSig,hardSig,industrialSig,raveSig) : ba.selectn(5,family) : *(outGain);
process = mono <: _,_;
