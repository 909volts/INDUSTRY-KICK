...
Write-Host "INDUSTRY KICK — Stage 7.2B DSP Freeze / Pre-GUI Gate"
Write-Host "runnerParserFix=7.2B"
...
# -------------------------------------------------------------------
# Windows PowerShell 5.1-safe hard-gate aggregation.
$hardChecks = @(
    ($Status.SOURCE_IDENTITY -eq "PASS")
    ($Status.CHECKPOINT -eq "PASS")
    ($Status.CLEAN_DSP_BUILD -eq "PASS")
    ($Status.DSP_SMOKE -eq "PASS")
    ($Status.FACTORY_50 -eq "PASS")
    ($Status.RANDOMIZER -eq "PASS")
    ($Status.STATE_ROUNDTRIP -eq "PASS")
    ($Status.VST3_BUILD -eq "PASS")
    (($Status.STANDALONE_BUILD -eq "PASS") -or ($Status.STANDALONE_BUILD -eq "NOT_CONFIGURED"))
)
...
if ($Status.PRE_GUI_DSP_GATE -eq "PASS") {
    Write-Host ""
    Write-Host "STAGE_7_2B_PRE_GUI_DSP_GATE_COMPLETE"
    exit 0
}
