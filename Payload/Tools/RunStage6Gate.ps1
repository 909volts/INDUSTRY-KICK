...
Write-Host "INDUSTRY KICK V2 - Stage 6.9 Measured Headroom + RAVE AR Build Gate"
...
Write-Host ""
Write-Host "STAGE_6_9_BUILD_GATE_COMPLETE"
$renderDir = Join-Path $ProjectRoot "Stage69TestRenders"
$renderZip = Join-Path $ProjectRoot "Stage69TestRenders.zip"
if (Test-Path $renderDir) {
    ...
    Write-Host "STAGE_6_9_RENDER_ZIP=$renderZip"
} else {
    Write-Host "STAGE_6_9_RENDER_ZIP=NOT_FOUND"
}
Write-Host "Send Stage69TestRenders.zip only if result=PASS."
